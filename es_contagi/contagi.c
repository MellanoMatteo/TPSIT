...
*/
{
    int max_ric1 = 0, max_ric2 = 0, max_ric3 = 0;
    int ind1 = 0, ind2 = 0, ind3 = 0;

    for (int i = 0; i < regioni; i++)
        if(max_ric1 < (italia+i)->ter_int)
        {
            ind1 = i;
            max_ric1 = (italia+i)->ter_int;
        }
            
    for(int i = 0; i < regioni; i++)
        if(i != ind1)
            if(max_ric2 < (italia+i)->ter_int)
            {
                ind2 = i;
                max_ric2 = (italia+i)->ter_int;
            }

    for(int i = 0; i < regioni; i++)
        if(i != ind1 && i != ind2)
            if(max_ric3 < (italia+i)->ter_int)
            {
                ind3 = i;
                max_ric3 = (italia+i)->ter_int;
            }  
